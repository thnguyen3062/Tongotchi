...
namespace DanielLochner.Assets.SimpleScrollSnap
{
    ...
    public class SimpleScrollSnap : MonoBehaviour, IDragHandler, IBeginDragHandler, IEndDragHandler, IPointerDownHandler, IPointerUpHandler
    {
        ...
        [SerializeField] private bool notRequireEqualToggle;
        ...
        public bool NotRequireEqualToggle
        {
            get => notRequireEqualToggle;
            set => notRequireEqualToggle = value;
        }
        ...
        private bool ValidConfig
        {
            get
            {
                ...
                if (pagination != null)
                {
                    ...
                    if (numberOfToggles != NumberOfPanels && !notRequireEqualToggle)
                    {
        ...
        private int currentCenteredPanel = -1;

        private void HandleNextPreviousButton()
        {
            if (NumberOfPanels == 0)
                return;
            ...
            if (currentCenteredPanel == CenteredPanel)
                return;

            nextButton.gameObject.SetActive(CenteredPanel != NumberOfPanels - 1);
            previousButton.gameObject.SetActive(CenteredPanel != 0);
            currentCenteredPanel = CenteredPanel;
        }
        ...
        public void GoToPanel(int panelNumber)
        {
            ...
            if (pagination != null)
            {
                if (notRequireEqualToggle)
                {
                    if (panelNumber == 0)
                        Toggles[0].isOn = true;
                    else if (panelNumber == NumberOfPanels - 1)
                        Toggles[2].isOn = true;
                    else
                        Toggles[1].isOn = true;
                }
        ...
        public void AddToBack(GameObject panel, bool canPool = false)
        {
            Add(panel, NumberOfPanels, canPool);
        }
        public void Add(GameObject panel, int index, bool canPool = false, OnSpawn callback = null)
        {
